...
// server cannot leak one caller's task ids to another. And `TaskRequest`
// went with `params.task`: the client no longer asks for a task at all,
// so there is no client-supplied task shape left to fuzz.
use bridge_mcp::{TaskCancelParams, TaskGetParams};
