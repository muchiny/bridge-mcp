...
| FIND-028 | `src/config/types.rs:219` | `HostConfig.sudo_password: Option<String>` not `Zeroizing`. Same class as FIND-014 (SOCKS password) on the sibling field. `HostConfig` lives for entire process lifetime; password sits in heap from start to exit. Hot-reload (`src/config/watcher.rs`) does NOT wipe old allocations. | `pub sudo_password: Option<Zeroizing<String>>`. Update borrow sites with `.as_deref()` — no behavior change. | Task 11 (zeroize-audit) | fixed (PENDING_SHA) — type wrapped in `Zeroizing<String>`; 4 production read sites compile unchanged via `Deref` coercion (`&Zeroizing<String>` → `&String` → `&str`); ~519 `None` fixture sites unchanged; one test fixture (`tests/e2e_docker.rs`) updated to wrap a literal sudo password; type-pinning regression test in `tests/sudo_password_zeroizing.rs` |
