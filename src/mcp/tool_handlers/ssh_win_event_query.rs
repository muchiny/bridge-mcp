...
impl StandardTool for WinEventQueryTool {
    ...
    fn post_process(
        ...
        tbl = tbl.action(
            ...
            "Refresh",
            "ssh_win_event_query",
            Some(json!({"host": args.host, "log": args.log})),
        );
...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshWinEventQueryArgs =
            serde_json::from_value(json!({"host": "s", "log": "Application"})).unwrap();
        let host = test_host_config();
        let cmd = WinEventQueryTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = WinEventQueryTool::post_process(result, &args, output, &dr);
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(Some(json!({"host": "winhost", "log": "System"})), &ctx)
            .await
            .unwrap();
