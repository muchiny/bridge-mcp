//! Handler for the `ssh_apparmor_profiles` tool.
//!
//! List `AppArmor` profiles loaded on a remote host.
...
pub struct SshApparmorProfilesArgs {
    host: String,
    #[serde(default)]
...
impl_common_args!(SshApparmorProfilesArgs);

#[mcp_standard_tool(
    name = "ssh_apparmor_profiles",
    group = "security_modules",
    annotation = "read_only"
)]
pub struct ApparmorProfilesTool;

impl StandardTool for ApparmorProfilesTool {
    type Args = SshApparmorProfilesArgs;
...
/// Handler for the `ssh_apparmor_profiles` tool.
pub type SshApparmorProfilesHandler = StandardToolHandler<ApparmorProfilesTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshApparmorProfilesHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_apparmor_profiles");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshApparmorProfilesArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshApparmorProfilesArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshApparmorProfilesArgs>(json);
        assert!(result.is_err());
    }
