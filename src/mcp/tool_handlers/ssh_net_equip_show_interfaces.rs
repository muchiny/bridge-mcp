//! SSH Network Equipment Show Interfaces Tool Handler
//!
//! Shows interface status on a network device via SSH.
...
impl StandardTool for NetEquipShowInterfacesTool {
    type Args = SshNetEquipShowInterfacesArgs;
    ...
    fn build_command(
        args: &SshNetEquipShowInterfacesArgs,
...
/// Handler for the `ssh_net_equip_show_interfaces` tool.
pub type SshNetEquipShowInterfacesHandler = StandardToolHandler<NetEquipShowInterfacesTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshNetEquipShowInterfacesHandler::new();
    ...
    fn test_schema() {
        let handler = SshNetEquipShowInterfacesHandler::new();
        assert_eq!(handler.name(), "ssh_net_equip_show_interfaces");
    ...
    fn test_build_command_defaults() {
        let args: SshNetEquipShowInterfacesArgs =
            serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = NetEquipShowInterfacesTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
