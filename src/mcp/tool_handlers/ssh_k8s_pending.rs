//! K8s Pending Tool Handler — list Pending pods with scheduler messages.
...
/// Arguments for the `ssh_k8s_pending` tool.
#[derive(Debug, Deserialize)]
pub struct SshK8sPendingArgs {
    host: String,
    #[serde(default)]
...
impl_common_args!(SshK8sPendingArgs);

/// Handler marker for the `ssh_k8s_pending` tool.
#[mcp_standard_tool(
    name = "ssh_k8s_pending",
    group = "kubernetes",
    annotation = "read_only"
)]
pub struct K8sPending;

impl StandardTool for K8sPending {
    type Args = SshK8sPendingArgs;
    const NAME: &'static str = "ssh_k8s_pending";
    const DESCRIPTION: &'static str = "K8s pending-pod investigator — lists all Pending pods \
        joined with their `FailedScheduling` events, so you can see scheduler messages \
        (resource pressure, affinity conflicts, etc.) alongside the pod details. \
    ...
    fn build_command(args: &SshK8sPendingArgs, _host_config: &HostConfig) -> Result<String> {
        K8sTriageCommandBuilder::build_pending_command(
...
/// Handler for the `ssh_k8s_pending` tool.
pub type SshK8sPendingHandler = StandardToolHandler<K8sPending>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshK8sPendingHandler::new();
    ...
    fn test_schema() {
        let handler = SshK8sPendingHandler::new();
        assert_eq!(handler.name(), "ssh_k8s_pending");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_k8s_pending");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshK8sPendingArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshK8sPendingArgs"));
    }
    ...
    #[test]
    fn test_build_command_pending_basic() {
        let args = SshK8sPendingArgs {
            host: "s1".into(),
            namespace: None,
        ...
        let cmd = K8sPending::build_command(&args, &test_host_config()).unwrap();
        assert!(cmd.contains("FailedScheduling"), "cmd: {cmd}");
        assert!(cmd.contains("schedulerMessages"), "cmd: {cmd}");
    }

    #[test]
    fn test_build_command_pending_with_namespace() {
        let args = SshK8sPendingArgs {
            host: "s1".into(),
