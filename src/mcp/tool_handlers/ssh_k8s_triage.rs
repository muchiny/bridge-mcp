//! K8s Triage Tool Handler — composite pod + event aggregation.
...
/// Arguments for the `ssh_k8s_triage` tool.
#[derive(Debug, Deserialize)]
pub struct SshK8sTriageArgs {
    host: String,
    #[serde(default)]
...
impl_common_args!(SshK8sTriageArgs);

/// Handler marker for the `ssh_k8s_triage` tool.
#[mcp_standard_tool(
    name = "ssh_k8s_triage",
    group = "kubernetes",
    annotation = "read_only"
)]
pub struct K8sTriage;

impl StandardTool for K8sTriage {
    type Args = SshK8sTriageArgs;
    const NAME: &'static str = "ssh_k8s_triage";
    const DESCRIPTION: &'static str = "K8s cluster triage — aggregates non-running pods and \
        Warning events into a single JSON object `{notReadyPods, warningEvents}`. Requires jq \
        on the host. Use `namespace` to scope, `all_namespaces` for cross-namespace view, or \
        omit both for the current context namespace.";
    ...
    fn build_command(args: &SshK8sTriageArgs, _host_config: &HostConfig) -> Result<String> {
        K8sTriageCommandBuilder::build_triage_command(
...
/// Handler for the `ssh_k8s_triage` tool.
pub type SshK8sTriageHandler = StandardToolHandler<K8sTriage>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshK8sTriageHandler::new();
    ...
    fn test_schema() {
        let handler = SshK8sTriageHandler::new();
        assert_eq!(handler.name(), "ssh_k8s_triage");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_k8s_triage");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshK8sTriageArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshK8sTriageArgs"));
    }
    ...
    #[test]
    fn test_build_command_triage_default() {
        let args = SshK8sTriageArgs {
            host: "s1".into(),
            namespace: None,
            all_namespaces: false,
            context: None,
        ...
        let cmd = K8sTriage::build_command(&args, &test_host_config()).unwrap();
    ...
    #[test]
    fn test_build_command_triage_with_namespace() {
        let args = SshK8sTriageArgs {
            host: "s1".into(),
