//! Handler for the `ssh_storage_fdisk` tool.
//!
//! Shows partition table information on a remote host (read-only).
...
pub struct SshStorageFdiskArgs {
    ...
    host: String,
    /// Optional device to inspect (e.g., /dev/sda).
    #[serde(default)]
...
impl StandardTool for StorageFdiskTool {
    ...
    const SCHEMA: &'static str = r#"{
                ...
                "properties": {
                    ...
                    "device": {
                        "type": "string",
                        "description": "Device to inspect (e.g., /dev/sda). Lists all devices if omitted"
                    },
                    "timeout_seconds": {
                        "type": "integer",
    ...
    fn build_command(args: &SshStorageFdiskArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(StorageCommandBuilder::build_fdisk_command(
...
/// Handler for the `ssh_storage_fdisk` tool.
pub type SshStorageFdiskHandler = StandardToolHandler<StorageFdiskTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshStorageFdiskHandler::new();
    ...
    fn test_schema() {
        let handler = SshStorageFdiskHandler::new();
        assert_eq!(handler.name(), "ssh_storage_fdisk");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_storage_fdisk");
    ...
    fn test_args_deserialization() {
        let json = json!({
            ...
            "timeout_seconds": 15,
            "max_output": 5000,
            "save_output": "/tmp/fdisk.txt"
        });
        let args: SshStorageFdiskArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "server1");
        assert_eq!(args.device.as_deref(), Some("/dev/sda"));
        assert_eq!(args.timeout_seconds, Some(15));
        assert_eq!(args.max_output, Some(5000));
        assert_eq!(args.save_output.as_deref(), Some("/tmp/fdisk.txt"));
    }
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshStorageFdiskArgs"));
    }
    ...
    fn test_build_command_defaults() {
        let args: SshStorageFdiskArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = StorageFdiskTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
        assert!(cmd.contains("fdisk"));
    }

    #[test]
    fn test_build_command_with_device() {
        let args: SshStorageFdiskArgs =
            serde_json::from_value(json!({"host": "s", "device": "/dev/sda"})).unwrap();
        let host = test_host_config();
        let cmd = StorageFdiskTool::build_command(&args, &host).unwrap();
    ...
    fn test_post_process_with_output() {
        ...
        let output = "Disk /dev/sda: 100 GiB\nSector size: 512\n";
        let result = StorageFdiskTool::post_process(result, &args, output, &dr);
    ...
    fn test_post_process_empty_output() {
        ...
        let args: SshStorageFdiskArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let dr = crate::domain::data_reduction::DataReductionArgs::default();
        let result = StorageFdiskTool::post_process(result, &args, "", &dr);
        assert!(!result.content.is_empty());
    }
