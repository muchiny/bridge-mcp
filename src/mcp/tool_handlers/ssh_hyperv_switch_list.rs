//! Handler for the `ssh_hyperv_switch_list` tool.
//!
//! Lists Hyper-V virtual switches via `PowerShell`.
...
pub struct SshHypervSwitchListArgs {
    host: String,
    #[serde(default)]
...
impl_common_args!(SshHypervSwitchListArgs);

#[mcp_standard_tool(
    name = "ssh_hyperv_switch_list",
    group = "hyperv",
    annotation = "read_only"
)]
pub struct HypervSwitchListTool;

impl StandardTool for HypervSwitchListTool {
    type Args = SshHypervSwitchListArgs;
    ...
    fn build_command(_args: &SshHypervSwitchListArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(HyperVCommandBuilder::switch_list())
    }

    fn post_process(
        result: ToolCallResult,
        args: &SshHypervSwitchListArgs,
        output: &str,
        ...
        let mut tbl = table("Hyper-V Switches");
...
/// Handler for the `ssh_hyperv_switch_list` tool.
pub type SshHypervSwitchListHandler = StandardToolHandler<HypervSwitchListTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshHypervSwitchListHandler::new();
    ...
    fn test_schema() {
        let handler = SshHypervSwitchListHandler::new();
        assert_eq!(handler.name(), "ssh_hyperv_switch_list");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshHypervSwitchListArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshHypervSwitchListArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshHypervSwitchListArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshHypervSwitchListArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = HypervSwitchListTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = HypervSwitchListTool::post_process(result, &args, output, &dr);
