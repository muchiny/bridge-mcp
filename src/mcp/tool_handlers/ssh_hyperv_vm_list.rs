//! Handler for the `ssh_hyperv_vm_list` tool.
//!
//! Lists Hyper-V virtual machines via `PowerShell`.
...
pub struct SshHypervVmListArgs {
    host: String,
    #[serde(default)]
...
#[mcp_standard_tool(
    name = "ssh_hyperv_vm_list",
    group = "hyperv",
    annotation = "read_only"
)]
pub struct HypervVmListTool;

impl StandardTool for HypervVmListTool {
    type Args = SshHypervVmListArgs;
    ...
    fn build_command(_args: &SshHypervVmListArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(HyperVCommandBuilder::vm_list())
    }

    fn post_process(
        result: ToolCallResult,
        args: &SshHypervVmListArgs,
        output: &str,
...
/// Handler for the `ssh_hyperv_vm_list` tool.
pub type SshHypervVmListHandler = StandardToolHandler<HypervVmListTool>;
...
mod tests {
    ...
    fn test_args_deserialization() {
        ...
        let args: SshHypervVmListArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshHypervVmListArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshHypervVmListArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = HypervVmListTool::post_process(result, &args, output, &dr);
