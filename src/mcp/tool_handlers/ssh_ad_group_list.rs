//! Handler for the `ssh_ad_group_list` tool.
//!
//! List all Active Directory groups with name, scope, and category.
...
impl_common_args!(SshAdGroupListArgs);

#[mcp_standard_tool(
    name = "ssh_ad_group_list",
    group = "active_directory",
    annotation = "read_only"
)]
pub struct AdGroupListTool;

impl StandardTool for AdGroupListTool {
    type Args = SshAdGroupListArgs;
    ...
    fn build_command(_args: &SshAdGroupListArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(ActiveDirectoryCommandBuilder::build_group_list_command())
    }

    fn post_process(
        result: ToolCallResult,
        args: &SshAdGroupListArgs,
        output: &str,
...
/// Handler for the `ssh_ad_group_list` tool.
pub type SshAdGroupListHandler = StandardToolHandler<AdGroupListTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshAdGroupListHandler::new();
    ...
    fn test_schema() {
        let handler = SshAdGroupListHandler::new();
        assert_eq!(handler.name(), "ssh_ad_group_list");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_ad_group_list");
    ...
    fn test_args_deserialization() {
        let json = json!({
            ...
            "timeout_seconds": 30,
            "max_output": 5000,
            "save_output": "/tmp/groups.txt"
        });
        let args: SshAdGroupListArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "windc01");
        assert_eq!(args.timeout_seconds, Some(30));
        assert_eq!(args.max_output, Some(5000));
        assert_eq!(args.save_output, Some("/tmp/groups.txt".to_string()));
    }
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshAdGroupListArgs"));
    }
    ...
    fn test_build_command_defaults() {
        let args: SshAdGroupListArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = AdGroupListTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = AdGroupListTool::post_process(result, &args, output, &dr);
