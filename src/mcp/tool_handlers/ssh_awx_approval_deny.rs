//! SSH AWX Approval Deny Tool Handler
//!
//! Denies a pending AWX workflow approval via REST API relayed through SSH.
...
/// Handler for the `ssh_awx_approval_deny` tool.
#[mcp_tool(
    name = "ssh_awx_approval_deny",
    group = "awx",
    annotation = "destructive"
)]
pub struct SshAwxApprovalDenyHandler;

impl Default for SshAwxApprovalDenyHandler {
    fn default() -> Self {
        Self::new()
    }
}

impl SshAwxApprovalDenyHandler {
    ...
    "properties": {
        "approval_id": {
            "type": "integer",
            "description": "AWX workflow approval ID to deny",
            "minimum": 1
        }
    },
...
#[async_trait]
impl ToolHandler for SshAwxApprovalDenyHandler {
    fn name(&self) -> &'static str {
        "ssh_awx_approval_deny"
    }

    fn description(&self) -> &'static str {
        "Deny a pending AWX workflow approval, causing its workflow branch to fail. Find the ID with ssh_awx_approvals."
    }
    ...
    async fn execute(&self, args: Option<Value>, ctx: &ToolContext) -> Result<ToolCallResult> {
        ...
        let args: SshAwxApprovalDenyArgs = serde_json::from_value(raw)
        ...
        let endpoint = format!("/api/v2/workflow_approvals/{}/deny/", args.approval_id);
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshAwxApprovalDenyHandler;
    ...
    fn test_schema() {
        let handler = SshAwxApprovalDenyHandler;
        assert_eq!(handler.name(), "ssh_awx_approval_deny");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_awx_approval_deny");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshAwxApprovalDenyArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshAwxApprovalDenyArgs"));
    }
