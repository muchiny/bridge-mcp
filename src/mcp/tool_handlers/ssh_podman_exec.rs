...
#[mcp_standard_tool(name = "ssh_podman_exec", group = "podman", annotation = "mutating")]
...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshPodmanExecArgs =
            serde_json::from_value(json!({"host": "s", "container": "myapp", "command": "ls"}))
                .unwrap();
        let host = test_host_config();
        let cmd = PodmanExecTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(
                Some(json!({"host": "server1", "container": "mycontainer", "command": "ls"})),
                &ctx,
            )
            .await
            .unwrap();
