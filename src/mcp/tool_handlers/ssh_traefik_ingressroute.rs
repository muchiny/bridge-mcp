...
impl_common_args!(SshTraefikIngressrouteArgs);

/// Handler marker for the `ssh_traefik_ingressroute` tool.
#[mcp_standard_tool(
    name = "ssh_traefik_ingressroute",
    group = "kubernetes",
    annotation = "read_only"
)]
pub struct TraefikIngressroute;

impl StandardTool for TraefikIngressroute {
    type Args = SshTraefikIngressrouteArgs;
    const NAME: &'static str = "ssh_traefik_ingressroute";
    const DESCRIPTION: &'static str = "Inspect a Traefik IngressRoute CRD: shows the full YAML, \
        routes (match rules → service:port + middlewares), referenced middleware details, \
        and backend service endpoints. Supports both traefik.io and traefik.containo.us API groups.";
    ...
    const SCHEMA: &'static str = r#"{
        ...
        "properties": {
            ...
            "route": {
                "type": "string",
                "description": "Name of the Traefik IngressRoute to inspect"
            },
            "namespace": {
                "type": "string",
    ...
    fn build_command(
        args: &SshTraefikIngressrouteArgs,
        _host_config: &HostConfig,
    ) -> Result<String> {
        KubernetesCommandBuilder::build_traefik_ingressroute_command(
...
/// Handler for the `ssh_traefik_ingressroute` tool.
pub type SshTraefikIngressrouteHandler = StandardToolHandler<TraefikIngressroute>;
...
mod tests {
    ...
    fn test_args_full_deserialization() {
        ...
        let args: SshTraefikIngressrouteArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "k8s-host");
        assert_eq!(args.route, "my-route");
        assert_eq!(args.namespace, Some("production".to_string()));
    }

    #[test]
    fn test_args_minimal_deserialization() {
        let json = json!({"host": "k8s", "route": "web-route"});
        let args: SshTraefikIngressrouteArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "k8s");
        assert_eq!(args.route, "web-route");
    ...
    fn test_args_debug() {
        ...
        let args: SshTraefikIngressrouteArgs = serde_json::from_value(json).unwrap();
        let s = format!("{args:?}");
        assert!(s.contains("SshTraefikIngressrouteArgs"));
    }

    #[test]
    fn test_schema_optional_fields() {
        let handler = SshTraefikIngressrouteHandler::new();
    ...
    async fn test_invalid_json_type() {
        ...
        let result = handler
            .execute(Some(json!({"host": 123, "route": "my-route"})), &ctx)
            .await;
    ...
    fn test_build_command_basic() {
        let args = SshTraefikIngressrouteArgs {
            host: "k8s".into(),
            route: "my-route".into(),
        ...
        let cmd = TraefikIngressroute::build_command(&args, &test_host_config()).unwrap();
    ...
    #[test]
    fn test_build_command_rejects_invalid_route() {
        let args = SshTraefikIngressrouteArgs {
            host: "k8s".into(),
            route: "BadRoute".into(),
            namespace: None,
        ...
        let result = TraefikIngressroute::build_command(&args, &test_host_config());
        assert!(result.is_err());
    }
