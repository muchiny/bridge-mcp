...
impl StandardTool for CanaryExecTool {
    ...
    fn pre_execute(
        ...
        std::future::ready(validate_canary_commands(args, ctx))
    }
...
mod tests {
    ...
    //
    // `command` and `health_check` are free-form shell fragments, exactly like
    // `ssh_exec`. Running them through the `StandardToolHandler` pipeline alone
    // would only apply `validate_builtin` (blacklist), silently exempting this
    // tool from the whitelist that `ssh_exec` is subject to.

    fn args_with(command: &str, health_check: Option<&str>) -> SshCanaryExecArgs {
        SshCanaryExecArgs {
    ...
    async fn test_command_outside_whitelist_is_denied() {
        ...
        let result = CanaryExecTool::pre_execute(&args_with("id", None), &ctx).await;
        assert!(
    ...
    async fn test_health_check_outside_whitelist_is_denied() {
        let ctx = crate::ports::mock::create_test_context_with_whitelist(&["^echo"]);
        let result = CanaryExecTool::pre_execute(&args_with("echo ok", Some("id")), &ctx).await;
        assert!(
            matches!(result, Err(BridgeError::CommandDenied { .. })),
    ...
    async fn test_whitelisted_command_is_allowed() {
        let ctx = crate::ports::mock::create_test_context_with_whitelist(&["^echo"]);
        let result =
            CanaryExecTool::pre_execute(&args_with("echo ok", Some("echo healthy")), &ctx).await;
