//! Handler for the `ssh_win_event_sources` tool.
//!
//! List available Windows Event Log sources with record count and last write time.
...
impl StandardTool for WinEventSourcesTool {
    type Args = SshWinEventSourcesArgs;
    ...
    fn build_command(_args: &SshWinEventSourcesArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(WindowsEventCommandBuilder::build_sources_command())
    }
}

/// Handler for the `ssh_win_event_sources` tool.
pub type SshWinEventSourcesHandler = StandardToolHandler<WinEventSourcesTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshWinEventSourcesHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_win_event_sources");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshWinEventSourcesArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshWinEventSourcesArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshWinEventSourcesArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshWinEventSourcesArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinEventSourcesTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
