//! Handler for the `ssh_group_add` tool.
//!
//! Creates a new group on a remote Linux host.
...
pub struct SshGroupAddArgs {
    /// Target host name from configuration.
    host: String,
    /// Group name to create.
    name: String,
    /// Explicit GID for the new group.
    #[serde(default)]
    gid: Option<u32>,
...
impl_common_args!(SshGroupAddArgs);
...
pub struct GroupAddTool;

impl StandardTool for GroupAddTool {
    type Args = SshGroupAddArgs;
    ...
    const SCHEMA: &'static str = r#"{
        ...
        "properties": {
            ...
            "gid": {
                "type": "integer",
                "description": "Explicit GID for the new group"
            },
            "system": {
                "type": "boolean",
                "description": "Create as a system group. Default: false"
            },
            "timeout_seconds": {
                "type": "integer",
    ...
    fn build_command(args: &SshGroupAddArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(UserCommandBuilder::build_group_add_command(
            &args.name,
            args.gid,
...
/// Handler for the `ssh_group_add` tool.
pub type SshGroupAddHandler = StandardToolHandler<GroupAddTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshGroupAddHandler::new();
    ...
    async fn test_unknown_host() {
        ...
        let result = handler
            .execute(
                Some(json!({"host": "nonexistent", "name": "developers"})),
                &ctx,
            )
            .await;
        assert!(result.is_err());
    ...
    fn test_schema() {
        let handler = SshGroupAddHandler::new();
        assert_eq!(handler.name(), "ssh_group_add");
    ...
    fn test_build_command_with_options() {
        let args: SshGroupAddArgs = serde_json::from_value(json!({
            "host": "s", "name": "developers", "gid": 2000, "system": true
        }))
        .unwrap();
        let host = test_host_config();
        let cmd = GroupAddTool::build_command(&args, &host).unwrap();
    ...
    fn test_post_process_with_output() {
        ...
        let output = "groupadd: group developers added\n";
        let result = GroupAddTool::post_process(result, &args, output, &dr);
    ...
    fn test_post_process_empty_output() {
        ...
        let args: SshGroupAddArgs =
            serde_json::from_value(json!({"host": "s", "name": "developers"})).unwrap();
        let dr = crate::domain::data_reduction::DataReductionArgs::default();
        let result = GroupAddTool::post_process(result, &args, "", &dr);
        assert!(!result.content.is_empty());
    }
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(Some(json!({"host": "server1", "name": "developers"})), &ctx)
            .await
            .unwrap();
