//! SSH AWX Activity Tool Handler
//!
//! Queries the AWX activity stream (audit trail) via REST API relayed
//! through SSH.
...
/// Arguments for `ssh_awx_activity` tool.
#[allow(dead_code)]
#[derive(Debug, Deserialize)]
struct SshAwxActivityArgs {
    #[serde(default)]
...
/// Handler for the `ssh_awx_activity` tool.
#[mcp_tool(name = "ssh_awx_activity", group = "awx", annotation = "read_only")]
pub struct SshAwxActivityHandler;

impl Default for SshAwxActivityHandler {
    fn default() -> Self {
        Self::new()
    }
}

impl SshAwxActivityHandler {
    ...
    const SCHEMA: &'static str = r#"{
        ...
        "properties": {
            "object_type": {
                "type": "string",
                "description": "Filter by object type (maps to query 'object1', e.g. 'job', 'host')"
            },
            "operation": {
                "type": "string",
                "description": "Filter by operation (e.g. 'create', 'update', 'delete')"
            },
            "actor": {
                "type": "string",
                "description": "Filter by actor username (maps to query 'actor__username')"
            },
            "page_size": {
                "type": "integer",
...
#[async_trait]
impl ToolHandler for SshAwxActivityHandler {
    fn name(&self) -> &'static str {
        "ssh_awx_activity"
    }

    fn description(&self) -> &'static str {
        "Audit trail: who launched/changed what. jq_filter='.results[] | \
         {timestamp,operation,object1,actor:.summary_fields.actor.username}'"
    }
    ...
    async fn execute(&self, args: Option<Value>, ctx: &ToolContext) -> Result<ToolCallResult> {
        ...
        if let Some(ref object_type) = args.object_type {
            query_params.push(("object1", object_type));
        }
        if let Some(ref operation) = args.operation {
            query_params.push(("operation", operation));
        }
        if let Some(ref actor) = args.actor {
            query_params.push(("actor__username", actor));
        }
        query_params.push(("order_by", "-timestamp"));
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshAwxActivityHandler;
    ...
    fn test_schema() {
        let handler = SshAwxActivityHandler;
        assert_eq!(handler.name(), "ssh_awx_activity");
        assert!(!handler.description().is_empty());

        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_awx_activity");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshAwxActivityArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.object_type, Some("job".to_string()));
        assert_eq!(args.operation, Some("create".to_string()));
        assert_eq!(args.actor, Some("admin".to_string()));
    ...
    fn test_args_minimal_deserialization() {
        ...
        let args: SshAwxActivityArgs = serde_json::from_value(json).unwrap();
        assert!(args.object_type.is_none());
    ...
    fn test_schema_optional_fields() {
        let schema_json: serde_json::Value =
            serde_json::from_str(SshAwxActivityHandler.schema().input_schema).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshAwxActivityArgs"));
    }
