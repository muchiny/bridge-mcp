...
impl StandardTool for WinFirewallAllowTool {
    ...
    const DESCRIPTION: &'static str = "Create a Windows Firewall inbound Allow rule (New-NetFirewallRule -Action Allow \
        -Direction Inbound) for a specific TCP/UDP port on a Windows host. Windows hosts only — \
        for Linux use ssh_firewall_allow instead. To block traffic use ssh_win_firewall_deny; \
...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshWinFirewallAllowArgs =
            serde_json::from_value(json!({"host": "s", "name": "TestRule", "port": 8080})).unwrap();
        let host = test_host_config();
        let cmd = WinFirewallAllowTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(
                Some(json!({"host": "winhost", "name": "AllowHTTP", "port": 80})),
                &ctx,
            )
            .await
            .unwrap();
