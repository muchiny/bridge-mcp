//! SSH Git Status Tool Handler
//!
//! Shows the working tree status of a Git repository on a remote host
//! via `git -C {path} status`.
...
impl StandardTool for GitStatusTool {
    type Args = SshGitStatusArgs;

    const NAME: &'static str = "ssh_git_status";

    const DESCRIPTION: &'static str = "Show the working tree status of a Git repository on a remote host. Returns modified, \
        staged, and untracked files. Use --short for compact output.";
    ...
    fn build_command(args: &SshGitStatusArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(GitCommandBuilder::build_status_command(
            &args.path,
...
/// Handler for the `ssh_git_status` tool.
pub type SshGitStatusHandler = StandardToolHandler<GitStatusTool>;
...
mod tests {
    ...
    fn test_schema() {
        let handler = SshGitStatusHandler::new();
        assert_eq!(handler.name(), "ssh_git_status");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_git_status");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshGitStatusArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_schema_optional_fields() {
        ...
        assert!(properties.contains_key("short"));
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshGitStatusArgs"));
    }
    ...
    fn test_build_command_defaults() {
        let args: SshGitStatusArgs =
            serde_json::from_value(json!({"host": "s", "path": "/opt/repo"})).unwrap();
        let host = test_host_config();
        let cmd = GitStatusTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
