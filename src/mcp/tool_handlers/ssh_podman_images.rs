...
#[mcp_standard_tool(name = "ssh_podman_images", group = "podman", annotation = "read_only")]
...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshPodmanImagesArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = PodmanImagesTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
