//! SSH Network Equipment Show Routes Tool Handler
//!
//! Shows routing table on a network device via SSH.
...
pub struct SshNetEquipShowRoutesArgs {
    host: String,
    #[serde(default)]
...
impl_common_args!(SshNetEquipShowRoutesArgs);

#[mcp_standard_tool(
    name = "ssh_net_equip_show_routes",
    group = "network_equipment",
    annotation = "read_only"
)]
pub struct NetEquipShowRoutesTool;

impl StandardTool for NetEquipShowRoutesTool {
    type Args = SshNetEquipShowRoutesArgs;
    ...
    fn build_command(
        args: &SshNetEquipShowRoutesArgs,
...
/// Handler for the `ssh_net_equip_show_routes` tool.
pub type SshNetEquipShowRoutesHandler = StandardToolHandler<NetEquipShowRoutesTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshNetEquipShowRoutesHandler::new();
    ...
    fn test_schema() {
        let handler = SshNetEquipShowRoutesHandler::new();
        assert_eq!(handler.name(), "ssh_net_equip_show_routes");
    ...
    fn test_build_command_defaults() {
        let args: SshNetEquipShowRoutesArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = NetEquipShowRoutesTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
