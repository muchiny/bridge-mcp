//! Handler for the `ssh_win_perf_memory` tool.
//!
//! Gets Windows memory performance counters on a remote host.
...
impl StandardTool for WinPerfMemoryTool {
    type Args = SshWinPerfMemoryArgs;
    ...
    fn build_command(_args: &SshWinPerfMemoryArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(WindowsPerfCommandBuilder::build_memory_command())
    }
}

/// Handler for the `ssh_win_perf_memory` tool.
pub type SshWinPerfMemoryHandler = StandardToolHandler<WinPerfMemoryTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshWinPerfMemoryHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_win_perf_memory");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshWinPerfMemoryArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshWinPerfMemoryArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshWinPerfMemoryArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshWinPerfMemoryArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinPerfMemoryTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
