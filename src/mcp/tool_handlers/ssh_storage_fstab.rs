//! Handler for the `ssh_storage_fstab` tool.
//!
//! Reads the /etc/fstab file on a remote host.
...
impl StandardTool for StorageFstabTool {
    ...
    fn build_command(_args: &SshStorageFstabArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(StorageCommandBuilder::build_fstab_command())
    }
}

/// Handler for the `ssh_storage_fstab` tool.
pub type SshStorageFstabHandler = StandardToolHandler<StorageFstabTool>;
...
mod tests {
    ...
    fn test_args_deserialization() {
        let json = json!({
            ...
            "timeout_seconds": 15,
            "max_output": 5000,
            "save_output": "/tmp/fstab.txt"
        });
        let args: SshStorageFstabArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "server1");
        assert_eq!(args.timeout_seconds, Some(15));
        assert_eq!(args.max_output, Some(5000));
        assert_eq!(args.save_output.as_deref(), Some("/tmp/fstab.txt"));
    }
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshStorageFstabArgs"));
    }
    ...
    fn test_post_process_with_output() {
        ...
        let output = "/dev/sda1 / ext4 defaults 0 1\n";
        let result = StorageFstabTool::post_process(result, &args, output, &dr);
    ...
    fn test_post_process_empty_output() {
        ...
        let args: SshStorageFstabArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let dr = crate::domain::data_reduction::DataReductionArgs::default();
        let result = StorageFstabTool::post_process(result, &args, "", &dr);
        assert!(!result.content.is_empty());
    }
