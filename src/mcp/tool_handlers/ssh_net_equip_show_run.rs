//! SSH Network Equipment Show Running Config Tool Handler
//!
//! Shows running configuration on a network device via SSH.
...
pub struct SshNetEquipShowRunArgs {
    host: String,
    #[serde(default)]
...
impl_common_args!(SshNetEquipShowRunArgs);

#[mcp_standard_tool(
    name = "ssh_net_equip_show_run",
    group = "network_equipment",
    annotation = "read_only"
)]
pub struct NetEquipShowRunTool;

impl StandardTool for NetEquipShowRunTool {
    type Args = SshNetEquipShowRunArgs;
    ...
    fn build_command(args: &SshNetEquipShowRunArgs, _host_config: &HostConfig) -> Result<String> {
        let eq_type = args
            .equipment_type
            .as_deref()
...
/// Handler for the `ssh_net_equip_show_run` tool.
pub type SshNetEquipShowRunHandler = StandardToolHandler<NetEquipShowRunTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshNetEquipShowRunHandler::new();
    ...
    fn test_schema() {
        let handler = SshNetEquipShowRunHandler::new();
        assert_eq!(handler.name(), "ssh_net_equip_show_run");
    ...
    fn test_build_command_defaults() {
        let args: SshNetEquipShowRunArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = NetEquipShowRunTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
