//! Handler for the `ssh_win_disk_usage` tool.
//!
//! Shows disk usage on a Windows host via `PowerShell`.
...
pub struct SshWinDiskUsageArgs {
    host: String,
...
#[mcp_standard_tool(
    name = "ssh_win_disk_usage",
    group = "windows_perf",
    annotation = "read_only"
)]
pub struct WinDiskUsageTool;

impl StandardTool for WinDiskUsageTool {
    type Args = SshWinDiskUsageArgs;
    ...
    fn build_command(_args: &SshWinDiskUsageArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(WindowsProcessCommandBuilder::disk_usage())
    }
}

/// Handler for the `ssh_win_disk_usage` tool.
pub type SshWinDiskUsageHandler = StandardToolHandler<WinDiskUsageTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshWinDiskUsageHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_win_disk_usage");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshWinDiskUsageArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshWinDiskUsageArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshWinDiskUsageArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshWinDiskUsageArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinDiskUsageTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
