//! Handler for the `ssh_storage_lvm` tool.
//!
//! Lists LVM physical volumes, volume groups, and logical volumes on a remote host.
...
impl_common_args!(SshStorageLvmArgs);

#[mcp_standard_tool(name = "ssh_storage_lvm", group = "storage", annotation = "read_only")]
pub struct StorageLvmTool;

impl StandardTool for StorageLvmTool {
    type Args = SshStorageLvmArgs;
    ...
    fn build_command(_args: &SshStorageLvmArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(StorageCommandBuilder::build_lvm_list_command())
    }
}

/// Handler for the `ssh_storage_lvm` tool.
pub type SshStorageLvmHandler = StandardToolHandler<StorageLvmTool>;
...
mod tests {
    ...
    fn test_args_deserialization() {
        let json = json!({
            ...
            "timeout_seconds": 30,
            "max_output": 5000,
            "save_output": "/tmp/lvm.txt"
        });
        let args: SshStorageLvmArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "server1");
        assert_eq!(args.timeout_seconds, Some(30));
        assert_eq!(args.max_output, Some(5000));
        assert_eq!(args.save_output.as_deref(), Some("/tmp/lvm.txt"));
    }
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshStorageLvmArgs"));
    }
    ...
    fn test_post_process_with_output() {
        ...
        let output = "PV\tVG\tLV\n/dev/sda1\tvg0\troot\n";
        let result = StorageLvmTool::post_process(result, &args, output, &dr);
    ...
    fn test_post_process_empty_output() {
        ...
        let args: SshStorageLvmArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let dr = crate::domain::data_reduction::DataReductionArgs::default();
        let result = StorageLvmTool::post_process(result, &args, "", &dr);
        assert!(!result.content.is_empty());
    }
