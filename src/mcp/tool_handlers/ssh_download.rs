    ...
    const SCHEMA: &'static str = r#"{
        ...
        "properties": {
            ...
            "verify_checksum": {
                "type": "boolean",
                "default": false,
                "description": "Compute a SHA256 checksum of the downloaded bytes and report it in the result. This is not a verification: no independent checksum is computed on the remote host to compare against, so it cannot detect corruption between the remote source and what was received. Rejected when mode is resume or append (only a partial hash could be computed there)."
            },
...
impl ToolHandler for SshDownloadHandler {
    ...
    async fn execute(&self, args: Option<Value>, ctx: &ToolContext) -> Result<ToolCallResult> {
        ...
        let transfer_mode = parse_transfer_mode_checked(
            mode_str,
            args.verify_checksum.unwrap_or(false),
            "received",
        )?;
