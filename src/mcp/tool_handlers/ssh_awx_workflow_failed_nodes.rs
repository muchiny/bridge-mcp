//! SSH AWX Workflow Failed Nodes Tool Handler
//!
//! Lists the workflow nodes of an AWX workflow job run via the REST API
//! relayed through SSH, so failed nodes can be isolated with a `jq_filter`.
...
/// Handler for the `ssh_awx_workflow_failed_nodes` tool.
#[mcp_tool(
    name = "ssh_awx_workflow_failed_nodes",
    group = "awx",
    annotation = "read_only"
)]
pub struct SshAwxWorkflowFailedNodesHandler;

impl Default for SshAwxWorkflowFailedNodesHandler {
    fn default() -> Self {
        Self::new()
    }
}

impl SshAwxWorkflowFailedNodesHandler {
...
#[async_trait]
impl ToolHandler for SshAwxWorkflowFailedNodesHandler {
    fn name(&self) -> &'static str {
        "ssh_awx_workflow_failed_nodes"
    }

    fn description(&self) -> &'static str {
        "Workflow nodes; reduce to failures with jq_filter='.results[] | select(.summary_fields.job.status==\"failed\") | {id, job:.summary_fields.job.id, name:.summary_fields.job.name}'"
    }
    ...
    async fn execute(&self, args: Option<Value>, ctx: &ToolContext) -> Result<ToolCallResult> {
        ...
        let args: SshAwxWorkflowFailedNodesArgs = serde_json::from_value(raw)
        ...
        let _ = write!(
            endpoint,
            "/api/v2/workflow_jobs/{}/workflow_nodes/",
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshAwxWorkflowFailedNodesHandler;
    ...
    fn test_schema() {
        let handler = SshAwxWorkflowFailedNodesHandler;
        assert_eq!(handler.name(), "ssh_awx_workflow_failed_nodes");
        assert!(!handler.description().is_empty());

        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_awx_workflow_failed_nodes");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshAwxWorkflowFailedNodesArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.workflow_job_id, 10);
    ...
    #[test]
    fn test_args_debug() {
        let json = json!({"workflow_job_id": 5});
        let args: SshAwxWorkflowFailedNodesArgs = serde_json::from_value(json).unwrap();
        let debug_str = format!("{args:?}");
        assert!(debug_str.contains("SshAwxWorkflowFailedNodesArgs"));
    }
