//! SSH AWX Workflow Cancel Tool Handler
//!
//! Cancels a running AWX workflow job via REST API relayed through SSH.
...
/// Handler for the `ssh_awx_workflow_cancel` tool.
#[mcp_tool(
    name = "ssh_awx_workflow_cancel",
    group = "awx",
    annotation = "destructive"
)]
pub struct SshAwxWorkflowCancelHandler;

impl Default for SshAwxWorkflowCancelHandler {
    fn default() -> Self {
        Self::new()
    }
}

impl SshAwxWorkflowCancelHandler {
    ...
    "properties": {
        "workflow_job_id": {
            "type": "integer",
            "description": "AWX workflow job ID to cancel",
            "minimum": 1
        }
    },
...
#[async_trait]
impl ToolHandler for SshAwxWorkflowCancelHandler {
    fn name(&self) -> &'static str {
        "ssh_awx_workflow_cancel"
    }

    fn description(&self) -> &'static str {
        "Cancel a running AWX workflow job, stopping its remaining nodes. Find the ID with ssh_awx_workflow_status."
    }
    ...
    async fn execute(&self, args: Option<Value>, ctx: &ToolContext) -> Result<ToolCallResult> {
        ...
        let args: SshAwxWorkflowCancelArgs = serde_json::from_value(raw)
        ...
        let endpoint = format!("/api/v2/workflow_jobs/{}/cancel/", args.workflow_job_id);
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshAwxWorkflowCancelHandler;
    ...
    fn test_schema() {
        let handler = SshAwxWorkflowCancelHandler;
        assert_eq!(handler.name(), "ssh_awx_workflow_cancel");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_awx_workflow_cancel");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshAwxWorkflowCancelArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_schema_optional_fields() {
        let schema_json: serde_json::Value =
            serde_json::from_str(SshAwxWorkflowCancelHandler.schema().input_schema).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshAwxWorkflowCancelArgs"));
    }
    ...
    fn test_endpoint_uses_id() {
        let endpoint = format!("/api/v2/workflow_jobs/{}/cancel/", 77u64);
        assert_eq!(endpoint, "/api/v2/workflow_jobs/77/cancel/");
    }

    #[tokio::test]
    async fn test_validate_workflow_job_zero() {
        let handler = SshAwxWorkflowCancelHandler;
        let ctx = create_test_context();
        let result = handler
