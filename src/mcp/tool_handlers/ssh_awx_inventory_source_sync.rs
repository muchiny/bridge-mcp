//! SSH AWX Inventory Source Sync Tool Handler
//!
//! Triggers a refresh of a dynamic AWX inventory source via REST API relayed
//! through SSH.
...
/// Handler for the `ssh_awx_inventory_source_sync` tool.
#[mcp_tool(
    name = "ssh_awx_inventory_source_sync",
    group = "awx",
    annotation = "mutating"
)]
pub struct SshAwxInventorySourceSyncHandler;

impl Default for SshAwxInventorySourceSyncHandler {
    fn default() -> Self {
        Self::new()
    }
}

impl SshAwxInventorySourceSyncHandler {
    ...
    "properties": {
        "inventory_source_id": {
            "type": "integer",
            "description": "AWX inventory source ID to refresh",
            "minimum": 1
        }
    },
    "required": ["inventory_source_id"]
}"#;
}

#[async_trait]
impl ToolHandler for SshAwxInventorySourceSyncHandler {
    fn name(&self) -> &'static str {
        "ssh_awx_inventory_source_sync"
    }

    fn description(&self) -> &'static str {
        "Trigger a refresh of a dynamic AWX inventory source, re-importing its hosts. Find the ID with ssh_awx_inventory_sources."
    }
    ...
    async fn execute(&self, args: Option<Value>, ctx: &ToolContext) -> Result<ToolCallResult> {
        ...
        let args: SshAwxInventorySourceSyncArgs = serde_json::from_value(raw)
            .map_err(|e| BridgeError::McpInvalidRequest(e.to_string()))?;

        AwxCommandBuilder::validate_id(args.inventory_source_id)?;
        ...
        let endpoint = format!(
            "/api/v2/inventory_sources/{}/update/",
...
mod tests {
    ...
    fn test_schema() {
        ...
        assert!(required.contains(&json!("inventory_source_id")));
    }

    #[test]
    fn test_args_deserialization() {
        let json = json!({"inventory_source_id": 42});
        let args: SshAwxInventorySourceSyncArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.inventory_source_id, 42);
    }

    #[test]
    fn test_args_minimal_deserialization() {
        let json = json!({"inventory_source_id": 1});
        let args: SshAwxInventorySourceSyncArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.inventory_source_id, 1);
    }
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshAwxInventorySourceSyncArgs"));
    }
    ...
    async fn test_invalid_json_type() {
        ...
        let ctx = create_test_context();
        let result = handler
            .execute(Some(json!({"inventory_source_id": "not_a_number"})), &ctx)
            .await;
        assert!(result.is_err());
        match result.unwrap_err() {
    ...
    async fn test_no_awx_config() {
        ...
        let ctx = create_test_context();
        let result = handler
            .execute(Some(json!({"inventory_source_id": 42})), &ctx)
            .await;
        assert!(result.is_err());
    ...
    #[tokio::test]
    async fn test_rejects_zero_id() {
        let handler = SshAwxInventorySourceSyncHandler;
        let ctx = create_test_context();
        let result = handler
            .execute(Some(json!({"inventory_source_id": 0})), &ctx)
            .await;
        assert!(result.is_err());
        match result.unwrap_err() {
            BridgeError::CommandDenied { reason } => {
                assert!(reason.contains("positive integer"), "got: {reason}");
            }
