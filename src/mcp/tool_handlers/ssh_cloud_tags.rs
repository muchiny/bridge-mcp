//! Handler for the `ssh_cloud_tags` tool.
//!
//! Retrieves cloud instance tags from a remote host.
...
pub struct SshCloudTagsArgs {
    /// Target host name from configuration.
    host: String,
    /// Cloud provider (aws, gcp, azure). Defaults to aws.
    #[serde(default)]
...
impl StandardTool for CloudTagsTool {
    type Args = SshCloudTagsArgs;
    ...
    const SCHEMA: &'static str = r#"{
                ...
                "properties": {
                    ...
                    "provider": {
                        "type": "string",
                        "description": "Cloud provider: aws, gcp, or azure (default: aws)",
                        "enum": ["aws", "gcp", "azure"]
                    },
                    "timeout_seconds": {
                        "type": "integer",
...
/// Handler for the `ssh_cloud_tags` tool.
pub type SshCloudTagsHandler = StandardToolHandler<CloudTagsTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshCloudTagsHandler::new();
    ...
    fn test_schema() {
        let handler = SshCloudTagsHandler::new();
        assert_eq!(handler.name(), "ssh_cloud_tags");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_cloud_tags");
    ...
    fn test_args_deserialization() {
        let json = json!({
            ...
            "timeout_seconds": 10,
            "max_output": 5000,
            "save_output": "/tmp/tags.json"
        });
        let args: SshCloudTagsArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "server1");
        assert_eq!(args.provider.as_deref(), Some("gcp"));
        assert_eq!(args.timeout_seconds, Some(10));
        assert_eq!(args.max_output, Some(5000));
        assert_eq!(args.save_output.as_deref(), Some("/tmp/tags.json"));
    }
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshCloudTagsArgs"));
    }
