...
impl StandardTool for WinProcessListTool {
    ...
    const DESCRIPTION: &'static str = "List all running processes on a Windows host via PowerShell. Shows PID, name, CPU, \
        memory, and start time sorted by CPU usage descending. Use ssh_win_process_by_name to \
        search by process name, or ssh_win_process_top for the N heaviest consumers. For Linux \
        hosts use ssh_process_list instead.";
    ...
    fn post_process(
        ...
        let mut tbl = table("Windows Processes");
...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshWinProcessListArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinProcessListTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = WinProcessListTool::post_process(result, &args, output, &dr);
