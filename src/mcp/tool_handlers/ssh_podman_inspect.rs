...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshPodmanInspectArgs =
            serde_json::from_value(json!({"host": "s", "target": "myapp"})).unwrap();
        let host = test_host_config();
        let cmd = PodmanInspectTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(
                Some(json!({"host": "server1", "target": "mycontainer"})),
                &ctx,
            )
            .await
            .unwrap();
