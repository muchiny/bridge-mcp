...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshPodmanLogsArgs =
            serde_json::from_value(json!({"host": "s", "container": "myapp"})).unwrap();
        let host = test_host_config();
        let cmd = PodmanLogsTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(
                Some(json!({"host": "server1", "container": "mycontainer"})),
                &ctx,
            )
            .await
            .unwrap();
