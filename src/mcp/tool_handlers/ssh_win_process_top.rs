//! Handler for the `ssh_win_process_top` tool.
//!
//! Shows top processes by CPU usage on a Windows host via `PowerShell`.
...
pub struct SshWinProcessTopArgs {
    host: String,
    count: Option<u32>,
...
#[mcp_standard_tool(
    name = "ssh_win_process_top",
    group = "windows_process",
    annotation = "read_only"
)]
pub struct WinProcessTopTool;

impl StandardTool for WinProcessTopTool {
    type Args = SshWinProcessTopArgs;
    ...
    fn build_command(args: &SshWinProcessTopArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(WindowsProcessCommandBuilder::top(args.count.unwrap_or(10)))
    }

    fn post_process(
        result: ToolCallResult,
        args: &SshWinProcessTopArgs,
        output: &str,
        ...
        let mut tbl = table("Windows Top Processes");
...
/// Handler for the `ssh_win_process_top` tool.
pub type SshWinProcessTopHandler = StandardToolHandler<WinProcessTopTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshWinProcessTopHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_win_process_top");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshWinProcessTopArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshWinProcessTopArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshWinProcessTopArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshWinProcessTopArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinProcessTopTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let output = "PID\tName\tCPU\n1234\tsvchost\t5.2\n";
        let result = WinProcessTopTool::post_process(result, &args, output, &dr);
