//! Handler for the `ssh_win_update_list` tool.
//!
//! Lists available Windows updates on a remote host.
...
pub struct SshWinUpdateListArgs {
    host: String,
...
#[mcp_standard_tool(
    name = "ssh_win_update_list",
    group = "windows_updates",
    annotation = "read_only"
)]
pub struct WinUpdateListTool;

impl StandardTool for WinUpdateListTool {
    type Args = SshWinUpdateListArgs;
    ...
    fn build_command(_args: &SshWinUpdateListArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(WindowsUpdateCommandBuilder::build_list_command())
    }

    fn post_process(
        result: ToolCallResult,
        args: &SshWinUpdateListArgs,
        output: &str,
        ...
        let mut tbl = table("Windows Updates");
...
/// Handler for the `ssh_win_update_list` tool.
pub type SshWinUpdateListHandler = StandardToolHandler<WinUpdateListTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshWinUpdateListHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_win_update_list");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshWinUpdateListArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshWinUpdateListArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshWinUpdateListArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshWinUpdateListArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinUpdateListTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = WinUpdateListTool::post_process(result, &args, output, &dr);
