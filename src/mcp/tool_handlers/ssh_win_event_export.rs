...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshWinEventExportArgs =
            serde_json::from_value(json!({"host": "s", "log": "System", "file": "C:\\out.evtx"}))
                .unwrap();
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(
                Some(json!({"host": "winhost", "log": "Application", "file": "C:\\events.evtx"})),
                &ctx,
            )
            .await
            .unwrap();
