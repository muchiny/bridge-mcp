//! Handler for the `ssh_iis_list_pools` tool.
//!
//! Lists all IIS application pools with name, state, and managed runtime version.
...
pub struct SshIisListPoolsArgs {
    host: String,
...
impl StandardTool for IisListPoolsTool {
    type Args = SshIisListPoolsArgs;
    ...
    fn build_command(_args: &SshIisListPoolsArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(IisCommandBuilder::build_list_pools_command())
    }

    fn post_process(
        result: ToolCallResult,
        args: &SshIisListPoolsArgs,
        output: &str,
...
/// Handler for the `ssh_iis_list_pools` tool.
pub type SshIisListPoolsHandler = StandardToolHandler<IisListPoolsTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshIisListPoolsHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_iis_list_pools");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshIisListPoolsArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshIisListPoolsArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshIisListPoolsArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = IisListPoolsTool::post_process(result, &args, output, &dr);
