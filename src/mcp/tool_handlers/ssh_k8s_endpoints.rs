//! SSH Kubernetes Endpoints Tool Handler
//!
//! Shows service selector ↔ pod readiness JOIN plus `EndpointSlice`
//! ready/notReady addresses for a given service.
...
/// Arguments for the `ssh_k8s_endpoints` tool.
#[derive(Debug, Deserialize)]
pub struct SshK8sEndpointsArgs {
    host: String,
    /// Service name.
    service: String,
    #[serde(default)]
...
impl_common_args!(SshK8sEndpointsArgs);

/// Handler marker for the `ssh_k8s_endpoints` tool.
#[mcp_standard_tool(
    name = "ssh_k8s_endpoints",
    group = "kubernetes",
    annotation = "read_only"
)]
pub struct K8sEndpoints;

impl StandardTool for K8sEndpoints {
    type Args = SshK8sEndpointsArgs;
    const NAME: &'static str = "ssh_k8s_endpoints";
    const DESCRIPTION: &'static str = "Inspect Kubernetes service endpoints: shows the service selector, pods matching \
        that selector with readiness status, and EndpointSlice ready/notReady addresses. \
        Useful for diagnosing 'no endpoints' errors and traffic routing issues.";
    ...
    const SCHEMA: &'static str = r#"{
        ...
        "properties": {
            ...
            "service": {
                "type": "string",
                "description": "Name of the Kubernetes service to inspect"
            },
            "namespace": {
                "type": "string",
    ...
    fn build_command(args: &SshK8sEndpointsArgs, _host_config: &HostConfig) -> Result<String> {
        KubernetesCommandBuilder::build_endpoints_command(
...
/// Handler for the `ssh_k8s_endpoints` tool.
pub type SshK8sEndpointsHandler = StandardToolHandler<K8sEndpoints>;
...
mod tests {
    ...
    fn test_args_full_deserialization() {
        ...
        let args: SshK8sEndpointsArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        let args: SshK8sEndpointsArgs = serde_json::from_value(json).unwrap();
        let s = format!("{args:?}");
        assert!(s.contains("SshK8sEndpointsArgs"));
    }

    #[test]
    fn test_schema_optional_fields() {
        let handler = SshK8sEndpointsHandler::new();
    ...
    fn test_build_command_basic() {
        ...
        let cmd = K8sEndpoints::build_command(&args, &test_host_config()).unwrap();
    ...
    fn test_build_command_rejects_invalid_service() {
        ...
        let result = K8sEndpoints::build_command(&args, &test_host_config());
        assert!(result.is_err());
    }
