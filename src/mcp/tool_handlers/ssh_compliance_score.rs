//! Handler for the `ssh_compliance_score` tool.
//!
//! Calculates a compliance score on a remote host.
...
pub struct SshComplianceScoreArgs {
...
#[mcp_standard_tool(
    name = "ssh_compliance_score",
    group = "compliance",
    annotation = "read_only"
)]
pub struct ComplianceScoreTool;

impl StandardTool for ComplianceScoreTool {
    type Args = SshComplianceScoreArgs;
    ...
    async fn enrich(
        ...
        let prompt = "You are a compliance auditor. Given this CIS/STIG score report, list the top 3 weakest control categories with their current score in bullet points. One line each, no preamble.";
...
/// Handler for the `ssh_compliance_score` tool.
pub type SshComplianceScoreHandler = StandardToolHandler<ComplianceScoreTool>;
...
mod tests {
    ...
    fn test_args_deserialization() {
        let json = json!({
            ...
            "timeout_seconds": 60,
            "max_output": 10000,
            "save_output": "/tmp/score.txt"
        });
        let args: SshComplianceScoreArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.host, "server1");
        assert_eq!(args.timeout_seconds, Some(60));
        assert_eq!(args.max_output, Some(10000));
        assert_eq!(args.save_output.as_deref(), Some("/tmp/score.txt"));
    }
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshComplianceScoreArgs"));
    }
