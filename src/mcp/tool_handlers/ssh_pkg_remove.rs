...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshPkgRemoveArgs =
            serde_json::from_value(json!({"host": "s", "name": "nginx"})).unwrap();
        let host = test_host_config();
        let cmd = PkgRemoveTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    ...
    fn test_build_command_with_pkg_manager() {
        let args: SshPkgRemoveArgs = serde_json::from_value(json!({
            "host": "s", "name": "nginx", "pkg_manager": "dnf"
        }))
        .unwrap();
        let host = test_host_config();
        let cmd = PkgRemoveTool::build_command(&args, &host).unwrap();
    ...
    fn test_post_process_with_output() {
        ...
        let output = "Removing nginx...\nDone.\n";
        let result = PkgRemoveTool::post_process(result, &args, output, &dr);
    ...
    fn test_post_process_empty_output() {
        ...
        let args: SshPkgRemoveArgs =
            serde_json::from_value(json!({"host": "s", "name": "nginx"})).unwrap();
        let dr = crate::domain::data_reduction::DataReductionArgs::default();
        let result = PkgRemoveTool::post_process(result, &args, "", &dr);
        assert!(!result.content.is_empty());
    }
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(Some(json!({"host": "server1", "name": "nginx"})), &ctx)
            .await
            .unwrap();
