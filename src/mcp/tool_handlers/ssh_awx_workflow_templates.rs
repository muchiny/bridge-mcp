//! SSH AWX Workflow Templates Tool Handler
//!
//! Lists AWX workflow job templates via REST API relayed through SSH.
...
/// Arguments for the `ssh_awx_workflow_templates` tool.
#[allow(dead_code)]
#[derive(Debug, Deserialize)]
struct SshAwxWorkflowTemplatesArgs {
    #[serde(default)]
...
impl SshAwxWorkflowTemplatesHandler {
    ...
    const SCHEMA: &'static str = r#"{
        ...
        "properties": {
            ...
            "search": {
                "type": "string",
                "description": "Search filter for workflow template names"
            },
            "timeout_seconds": {
                "type": "integer",
...
#[async_trait]
impl ToolHandler for SshAwxWorkflowTemplatesHandler {
    fn name(&self) -> &'static str {
        "ssh_awx_workflow_templates"
    }

    fn description(&self) -> &'static str {
        "List AWX workflow job templates (WJTs). Returns template names, IDs, and descriptions. \
         Launch one with ssh_awx_workflow_launch. Use jq_filter to extract fields \
         (e.g. '.results[] | {id, name}')."
    }
    ...
    async fn execute(&self, args: Option<Value>, ctx: &ToolContext) -> Result<ToolCallResult> {
        ...
        let args: SshAwxWorkflowTemplatesArgs = serde_json::from_value(raw)
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshAwxWorkflowTemplatesHandler;
    ...
    fn test_schema() {
        let handler = SshAwxWorkflowTemplatesHandler;
        assert_eq!(handler.name(), "ssh_awx_workflow_templates");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_awx_workflow_templates");
    ...
    fn test_args_deserialization() {
        let json = json!({"page_size": 25, "search": "deploy", "timeout_seconds": 60});
        let args: SshAwxWorkflowTemplatesArgs = serde_json::from_value(json).unwrap();
        assert_eq!(args.page_size, Some(25));
        assert_eq!(args.search.as_deref(), Some("deploy"));
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshAwxWorkflowTemplatesArgs"));
    }
