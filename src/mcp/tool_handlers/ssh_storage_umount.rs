...
mod tests {
    ...
    #[test]
    fn test_build_command_lazy() {
        let args: SshStorageUmountArgs =
            serde_json::from_value(json!({"host": "s", "path": "/mnt/data", "lazy": true}))
                .unwrap();
        let host = test_host_config();
        let cmd = StorageUmountTool::build_command(&args, &host).unwrap();
    ...
    fn test_post_process_with_output() {
        ...
        let output = "umount: /mnt/data unmounted\n";
        let result = StorageUmountTool::post_process(result, &args, output, &dr);
    ...
    fn test_post_process_empty_output() {
        ...
        let args: SshStorageUmountArgs =
            serde_json::from_value(json!({"host": "s", "path": "/mnt/data"})).unwrap();
        let dr = crate::domain::data_reduction::DataReductionArgs::default();
        let result = StorageUmountTool::post_process(result, &args, "", &dr);
        assert!(!result.content.is_empty());
    }
    ...
    async fn test_full_pipeline_success() {
        let handler = SshStorageUmountHandler::new();
        let ctx = pipeline_ctx(mock_output("umount: /mnt/data unmounted"));
        let result = handler
            .execute(Some(json!({"host": "server1", "path": "/mnt/data"})), &ctx)
            .await
            .unwrap();
