//! Handler for the `ssh_selinux_status` tool.
//!
//! Show `SELinux` status on a remote host.
...
impl StandardTool for SelinuxStatusTool {
    type Args = SshSelinuxStatusArgs;
...
/// Handler for the `ssh_selinux_status` tool.
pub type SshSelinuxStatusHandler = StandardToolHandler<SelinuxStatusTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshSelinuxStatusHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_selinux_status");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshSelinuxStatusArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshSelinuxStatusArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshSelinuxStatusArgs>(json);
        assert!(result.is_err());
    }
