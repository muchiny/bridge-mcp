//! SSH Network Equipment Show VLANs Tool Handler
//!
//! Shows VLAN configuration on a network device via SSH.
...
pub struct SshNetEquipShowVlansArgs {
    host: String,
    #[serde(default)]
...
#[mcp_standard_tool(
    name = "ssh_net_equip_show_vlans",
    group = "network_equipment",
    annotation = "read_only"
)]
pub struct NetEquipShowVlansTool;

impl StandardTool for NetEquipShowVlansTool {
    type Args = SshNetEquipShowVlansArgs;
...
/// Handler for the `ssh_net_equip_show_vlans` tool.
pub type SshNetEquipShowVlansHandler = StandardToolHandler<NetEquipShowVlansTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshNetEquipShowVlansHandler::new();
    ...
    fn test_schema() {
        let handler = SshNetEquipShowVlansHandler::new();
        assert_eq!(handler.name(), "ssh_net_equip_show_vlans");
    ...
    fn test_build_command_defaults() {
        let args: SshNetEquipShowVlansArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = NetEquipShowVlansTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
