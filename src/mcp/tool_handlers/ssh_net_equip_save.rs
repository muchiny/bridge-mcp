//! SSH Network Equipment Save Tool Handler
//!
//! Saves running configuration to persistent storage on a network device via SSH.
...
pub struct SshNetEquipSaveArgs {
    host: String,
    #[serde(default)]
...
impl_common_args!(SshNetEquipSaveArgs);

#[mcp_standard_tool(
    name = "ssh_net_equip_save",
    group = "network_equipment",
    annotation = "mutating"
)]
pub struct NetEquipSaveTool;

impl StandardTool for NetEquipSaveTool {
    type Args = SshNetEquipSaveArgs;
    ...
    fn build_command(args: &SshNetEquipSaveArgs, _host_config: &HostConfig) -> Result<String> {
        let eq_type = args
            .equipment_type
            .as_deref()
            .map_or(EquipmentType::Generic, EquipmentType::from_str_loose);
        Ok(NetworkEquipmentCommandBuilder::build_save_command(eq_type))
    }
}

/// Handler for the `ssh_net_equip_save` tool.
pub type SshNetEquipSaveHandler = StandardToolHandler<NetEquipSaveTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshNetEquipSaveHandler::new();
    ...
    fn test_schema() {
        let handler = SshNetEquipSaveHandler::new();
        assert_eq!(handler.name(), "ssh_net_equip_save");
    ...
    fn test_build_command_defaults() {
        let args: SshNetEquipSaveArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = NetEquipSaveTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
