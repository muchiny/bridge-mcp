//! Handler for the `ssh_win_net_adapters` tool.
//!
//! Lists network adapters on a remote Windows host via `PowerShell`.
...
pub struct SshWinNetAdaptersArgs {
    host: String,
...
impl_common_args!(SshWinNetAdaptersArgs);

#[mcp_standard_tool(
    name = "ssh_win_net_adapters",
    group = "windows_network",
    annotation = "read_only"
)]
pub struct WinNetAdaptersTool;

impl StandardTool for WinNetAdaptersTool {
    type Args = SshWinNetAdaptersArgs;
...
/// Handler for the `ssh_win_net_adapters` tool.
pub type SshWinNetAdaptersHandler = StandardToolHandler<WinNetAdaptersTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshWinNetAdaptersHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_win_net_adapters");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshWinNetAdaptersArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshWinNetAdaptersArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshWinNetAdaptersArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshWinNetAdaptersArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinNetAdaptersTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = WinNetAdaptersTool::post_process(result, &args, output, &dr);
