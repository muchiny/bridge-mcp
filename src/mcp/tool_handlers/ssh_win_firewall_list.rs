//! Handler for the `ssh_win_firewall_list` tool.
//!
//! Lists enabled Windows Firewall rules on a remote host.
...
pub struct SshWinFirewallListArgs {
    host: String,
...
#[mcp_standard_tool(
    name = "ssh_win_firewall_list",
    group = "windows_firewall",
    annotation = "read_only"
)]
pub struct WinFirewallListTool;

impl StandardTool for WinFirewallListTool {
    type Args = SshWinFirewallListArgs;
    ...
    fn build_command(_args: &SshWinFirewallListArgs, _host_config: &HostConfig) -> Result<String> {
        Ok(WindowsFirewallCommandBuilder::build_list_command())
    }

    fn post_process(
        result: ToolCallResult,
        args: &SshWinFirewallListArgs,
        output: &str,
        ...
        let mut tbl = table("Windows Firewall Rules");
...
/// Handler for the `ssh_win_firewall_list` tool.
pub type SshWinFirewallListHandler = StandardToolHandler<WinFirewallListTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshWinFirewallListHandler::new();
    ...
    fn test_schema() {
        ...
        assert_eq!(schema.name, "ssh_win_firewall_list");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshWinFirewallListArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug.contains("SshWinFirewallListArgs"));
    }

    #[test]
    fn test_invalid_json_type() {
        let json = json!({"host": 123});
        let result = serde_json::from_value::<SshWinFirewallListArgs>(json);
        assert!(result.is_err());
    }
    ...
    fn test_build_command_defaults() {
        let args: SshWinFirewallListArgs = serde_json::from_value(json!({"host": "s"})).unwrap();
        let host = test_host_config();
        let cmd = WinFirewallListTool::build_command(&args, &host).unwrap();
        assert!(!cmd.is_empty());
    }
    ...
    fn test_post_process_with_output() {
        ...
        let result = WinFirewallListTool::post_process(result, &args, output, &dr);
