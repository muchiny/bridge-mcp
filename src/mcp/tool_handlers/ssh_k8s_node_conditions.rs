...
/// Arguments for the `ssh_k8s_node_conditions` tool.
#[derive(Debug, Deserialize)]
pub struct SshK8sNodeConditionsArgs {
    host: String,
    #[serde(default)]
    node: Option<String>,
    #[serde(default)]
...
impl_common_args!(SshK8sNodeConditionsArgs);

/// Handler marker for the `ssh_k8s_node_conditions` tool.
#[mcp_standard_tool(
    name = "ssh_k8s_node_conditions",
    group = "kubernetes",
    annotation = "read_only"
)]
pub struct K8sNodeConditionsTool;
...
impl StandardTool for K8sNodeConditionsTool {
    ...
    fn build_command(args: &SshK8sNodeConditionsArgs, _host_config: &HostConfig) -> Result<String> {
        ...
        Ok(KubernetesCommandBuilder::build_node_conditions_command(
...
/// Handler for the `ssh_k8s_node_conditions` tool.
pub type SshK8sNodeConditionsHandler = StandardToolHandler<K8sNodeConditionsTool>;
...
mod tests {
    ...
    async fn test_missing_arguments() {
        let handler = SshK8sNodeConditionsHandler::new();
    ...
    fn test_schema() {
        let handler = SshK8sNodeConditionsHandler::new();
        assert_eq!(handler.name(), "ssh_k8s_node_conditions");
        assert!(!handler.description().is_empty());
        let schema = handler.schema();
        assert_eq!(schema.name, "ssh_k8s_node_conditions");
    ...
    fn test_args_deserialization() {
        ...
        let args: SshK8sNodeConditionsArgs = serde_json::from_value(json).unwrap();
    ...
    fn test_args_debug() {
        ...
        assert!(debug_str.contains("SshK8sNodeConditionsArgs"));
    }
    ...
    fn test_build_command_all_nodes() {
        let args = SshK8sNodeConditionsArgs {
            host: "s1".into(),
            node: None,
            context: None,
